...
def main():
    ...
    l40 = parse_layer(text, 40)
    l41 = parse_layer(text, 41)
    l42 = parse_layer(text, 42)
    l43 = parse_layer(text, 43)
    l44 = parse_layer(text, 44)

    in_buf = make_input(args.in_h * args.in_w * int(l40["C_in"]))
    a0_buf, a0_h, a0_w = qconv_hwc(l40, arrays, in_buf, args.in_h, args.in_w)
    mid_buf, mid_h, mid_w = qconv_hwc(l41, arrays, a0_buf, a0_h, a0_w)
    body_buf, body_h, body_w = qconv_hwc(l42, arrays, mid_buf, mid_h, mid_w)
    if body_h != a0_h or body_w != a0_w:
        raise SystemExit("C3 bottleneck body changed spatial shape")
    res_lut = make_res_lut(parse_f32(l41["act_in_scale"]), parse_f32(l42["act_out_scale"]))
    a_buf = add_residual(a0_buf, body_buf, res_lut)
    b_buf, b_h, b_w = qconv_hwc(l43, arrays, in_buf, args.in_h, args.in_w)
    if b_h != a0_h or b_w != a0_w:
        raise SystemExit("C3 branch mismatch")
    cat_buf = concat_hwc(a_buf, int(l42["K_out"]), b_buf, int(l43["K_out"]))
    out_buf, out_h, out_w = qconv_hwc(l44, arrays, cat_buf, a0_h, a0_w)

    tag = "L40_44"
    guard = f"YOLOV5N_{tag}_CASE0_H"

    with open(args.out, "w") as f:
        f.write("/* AUTO-GENERATED by tools/gen_yolo_c3_case.py — DO NOT EDIT */\n")
        f.write(f"#ifndef {guard}\n#define {guard}\n\n")
        f.write("#include <stdint.h>\n\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_INPUT_H {args.in_h}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_INPUT_W {args.in_w}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_INPUT_C {int(l40['C_in'])}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_OUTPUT_H {out_h}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_OUTPUT_W {out_w}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_OUTPUT_C {int(l44['K_out'])}u\n")
