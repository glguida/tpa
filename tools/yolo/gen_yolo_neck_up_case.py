...
def main():
    ...
    l34 = parse_layer(text, 34)
    l35 = parse_layer(text, 35)
    l36 = parse_layer(text, 36)
    l37 = parse_layer(text, 37)
    l38 = parse_layer(text, 38)
    l39 = parse_layer(text, 39)
    ...
    top_in = make_input(top_h * top_w * top_c, 0x6D2B79F5)
    skip_in = make_input(skip_h * skip_w * skip_c, 0x31415926)

    up_buf, up_h, up_w = upsample2x_nearest_hwc(top_in, top_h, top_w, top_c)
    if up_h != skip_h or up_w != skip_w:
        raise SystemExit("upsample shape mismatch")
    cat0_buf = concat_hwc(up_buf, top_c, skip_in, skip_c)

    a0_buf, a0_h, a0_w = qconv_hwc(l34, arrays, cat0_buf, skip_h, skip_w)
    mid_buf, mid_h, mid_w = qconv_hwc(l35, arrays, a0_buf, a0_h, a0_w)
    body_buf, body_h, body_w = qconv_hwc(l36, arrays, mid_buf, mid_h, mid_w)
    if body_h != a0_h or body_w != a0_w:
        raise SystemExit("neck_up bottleneck body changed spatial shape")
    res_lut = make_res_lut(parse_f32(l35["act_in_scale"]), parse_f32(l36["act_out_scale"]))
    a_buf = add_residual(a0_buf, body_buf, res_lut)
    b_buf, b_h, b_w = qconv_hwc(l37, arrays, cat0_buf, skip_h, skip_w)
    if b_h != a0_h or b_w != a0_w:
        raise SystemExit("neck_up branch mismatch")
    cat1_buf = concat_hwc(a_buf, int(l36["K_out"]), b_buf, int(l37["K_out"]))
    pre_buf, pre_h, pre_w = qconv_hwc(l38, arrays, cat1_buf, a0_h, a0_w)
    out_buf, out_h, out_w = qconv_hwc(l39, arrays, pre_buf, pre_h, pre_w)

    tag = "L34_39"
    guard = f"YOLOV5N_{tag}_CASE0_H"

    with open(args.out, "w") as f:
        f.write("/* AUTO-GENERATED by tools/gen_yolo_neck_up_case.py — DO NOT EDIT */\n")
        f.write(f"#ifndef {guard}\n#define {guard}\n\n")
        f.write("#include <stdint.h>\n\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_TOP_H {top_h}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_TOP_W {top_w}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_TOP_C {top_c}u\n")
        ...
        f.write(f"#define YOLOV5N_{tag}_CASE0_OUTPUT_C {int(l39['K_out'])}u\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_TOP_HASH UINT64_C(0x{fnv1a64(top_in):016x})\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_SKIP_HASH UINT64_C(0x{fnv1a64(skip_in):016x})\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_UP_HASH UINT64_C(0x{fnv1a64(up_buf):016x})\n")
        ...
        f.write(f"#define YOLOV5N_{tag}_CASE0_PRE_HASH UINT64_C(0x{fnv1a64(pre_buf):016x})\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_OUTPUT_HASH UINT64_C(0x{fnv1a64(out_buf):016x})\n")
        f.write(f"#define YOLOV5N_{tag}_CASE0_TOP_ELEMS {len(top_in)}u\n")
        ...
        emit_i8_array(f, "yolov5n_l34_39_case0_top", top_in)
        emit_i8_array(f, "yolov5n_l34_39_case0_skip", skip_in)
        emit_i8_array(f, "yolov5n_l34_39_res_lut", res_lut)
        emit_i8_array(f, "yolov5n_l34_39_case0_out", out_buf)
