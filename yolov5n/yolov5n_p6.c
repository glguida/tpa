...
#define BOX_C      64u
#define CLS_C      80u
#define YV5N_P6_ACC_BYTES    (sizeof(int32_t) * PIX_BLOCK * OC_BLOCK)
#define YV5N_P6_PACKED_BYTES (PIX_BLOCK * 128u)
#define YV5N_P6_PATCH_BYTES  (PIX_BLOCK * 768u)
#define YV5N_P6_SCRATCH_PEAK_BYTES \
    (YV5N_ARENA_STEP(0u, NR_PIX * CLS_C, 64u) + \
     (NR_PIX * CLS_C) + \
     YV5N_P6_ACC_BYTES + \
     YV5N_P6_PACKED_BYTES + \
     YV5N_P6_PATCH_BYTES)
...
tpa_op_t yolo_subgraph_p6_send_cls(void);
...
TPA_PROC_MEM_META(yolov5n_p6_mem_meta, 506u, YV5N_P6_SCRATCH_PEAK_BYTES);

static const yolov5n_layer_meta_t layer57_meta = {
    .lut = yv5n_l57_58_69_63_64_72_model_24_cv2_0_0_lut,
    .c_in = 64u,
    .k_out = 64u,
    .k_h = 3u,
    .k_w = 3u,
    .act_in_scale = 1.74605340e-02f,
    .act_out_scale = 2.23617554e-02f,
};

static const yolov5n_layer_meta_t layer58_meta = {
    .lut = yv5n_l57_58_69_63_64_72_model_24_cv2_0_1_lut,
    .c_in = 64u,
    .k_out = 64u,
    .k_h = 3u,
    .k_w = 3u,
    .act_in_scale = 2.23617554e-02f,
    .act_out_scale = 1.06447085e-01f,
};

static const yolov5n_layer_meta_t layer69_meta = {
    .lut = NULL,
    .c_in = 64u,
    .k_out = 64u,
    .k_h = 1u,
    .k_w = 1u,
    .act_in_scale = 1.06447085e-01f,
    .act_out_scale = 9.94826565e-02f,
};

static const yolov5n_layer_meta_t layer63_meta = {
    ...
    .k_w = 3u,
    .act_in_scale = 1.74605340e-02f,
    .act_out_scale = 1.80744731e-02f,
};

static const yolov5n_layer_meta_t layer64_meta = {
    .lut = yv5n_l57_58_69_63_64_72_model_24_cv3_0_1_lut,
    .c_in = 80u,
    .k_out = 80u,
    .k_h = 3u,
    .k_w = 3u,
    .act_in_scale = 1.80744731e-02f,
    .act_out_scale = 7.28613561e-02f,
    .exact_post = 1u,
};

static const yolov5n_layer_meta_t layer72_meta = {
    .lut = NULL,
    .c_in = 80u,
    .k_out = 80u,
    .k_h = 1u,
    .k_w = 1u,
    .act_in_scale = 7.28613561e-02f,
    .act_out_scale = 1.47422490e-01f,
    .exact_post = 1u,
};
...
tpa_op_t yolo_subgraph_p6_start(void)
{
    ...
    return tpa_recv(tpa_chan(0), (void **)&w->in, &w->in_len, yolo_subgraph_p6_run);
}
...
tpa_op_t yolo_subgraph_p6_run(void)
{
    ...
    YV5N_REQUIRE_MSG_STOP("P6:LEN\n", w->in_len == INPUT_H * INPUT_W * INPUT_C);

    yolov5n_arena_begin(YV5N_P6_SCRATCH_PEAK_BYTES);
    ...
    YV5N_REQUIRE_MSG_STOP("P6:ALLOC\n", det_stage0_buf && det_stage1_buf &&
                          acc_blk_scratch && packed_scratch && patch_scratch);

    enable_tensor_scratchpad();
    YV5N_TRY_MSG_STOP("P6:L57\n", run_conv3x3_layer_packed(&layer57, w->in, 64u, box1_buf, 64u));
    YV5N_TRY_MSG_STOP("P6:L58\n", run_conv3x3_layer_packed(&layer58, box1_buf, 64u, box2_buf, 64u));
    if (demo_outputs) {
        w->demo_box_out = yolov5n_send_buf(tpa_chan(1));
        YV5N_TRY_MSG_STOP("P6:L69D\n", run_conv1x1_layer(&layer69, box2_buf, 64u, w->demo_box_out, 64u));
    } else {
        YV5N_TRY_MSG_STOP("P6:L69\n", run_conv1x1_layer(&layer69, box2_buf, 64u, box_buf, 64u));
    }
    YV5N_TRY_MSG_STOP("P6:L63\n", run_conv3x3_layer_packed(&layer63, w->in, 64u, cls1_buf, 80u));
    YV5N_TRY_MSG_STOP("P6:L64\n", run_conv3x3_layer_packed(&layer64, cls1_buf, 80u, cls2_buf, 80u));
    if (demo_outputs) {
        w->demo_cls_out = yolov5n_send_buf(tpa_chan(2));
        YV5N_TRY_MSG_STOP("P6:L72D\n", run_conv1x1_layer(&layer72, cls2_buf, 80u, w->demo_cls_out, 80u));
    } else {
        YV5N_TRY_MSG_STOP("P6:L72\n", run_conv1x1_layer(&layer72, cls2_buf, 80u, cls_buf, 80u));
    }

    if (demo_outputs) {
        return tpa_send(tpa_chan(1), w->demo_box_out, YV5N_DEMO_P6_BOX_BYTES,
                        yolo_subgraph_p6_send_cls);
    }
...
tpa_op_t yolo_subgraph_p6_send_cls(void)
{
    ...
    return tpa_send(tpa_chan(2), w->demo_cls_out, YV5N_DEMO_P6_CLS_BYTES,
                    yolo_subgraph_p6_done);
}
