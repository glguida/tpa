...
#define YV5N_P5_PATCH_BYTES  (PIX_BLOCK * 288u)
#define YV5N_P5_SCRATCH_PEAK_BYTES \
...
TPA_PROC_MEM_META(yolov5n_p5_mem_meta, 505u, YV5N_P5_SCRATCH_PEAK_BYTES);
static const yolov5n_layer_meta_t layer40_meta = {
...
static const yolov5n_layer_meta_t layer41_meta = {
    .lut = yv5n_l40_44_model_17_m_0_cv1_lut,
    .c_in = 32u,
    .k_out = 32u,
    .k_h = 1u,
    .k_w = 1u,
    .act_in_scale = 1.95962350e-02f,
    .act_out_scale = 2.57143223e-02f,
};

static const yolov5n_layer_meta_t layer42_meta = {
    .lut = yv5n_l40_44_model_17_m_0_cv2_lut,
    .c_in = 32u,
    .k_out = 32u,
    .k_h = 3u,
    .k_w = 3u,
    .act_in_scale = 2.57143223e-02f,
    .act_out_scale = 2.32371458e-02f,
};

static const yolov5n_layer_meta_t layer43_meta = {
    ...
    .k_w = 1u,
    .act_in_scale = 2.44522282e-02f,
    .act_out_scale = 1.69020638e-02f,
};

static const yolov5n_layer_meta_t layer44_meta = {
    .lut = yv5n_l40_44_model_17_cv3_lut,
    .c_in = 64u,
    .k_out = 64u,
    .k_h = 1u,
    .k_w = 1u,
    .act_in_scale = 2.32371458e-02f,
    .act_out_scale = 1.74605340e-02f,
};
...
static void ensure_residual_lut(void)
{
    ...
    float src_scale = layer40.ld->act_out_scale;
    float dst_scale = layer42.ld->act_out_scale;
                             ...
                             int8_t *dst, uint32_t dst_c)
{
    ...
    for (size_t pix = 0; pix < NR_PIX; pix += PIX_BLOCK) {
        pack_3x3_block(src, src_c, pix, patch_scratch, layer->k_padded);
...
tpa_op_t yolo_subgraph_p5_recv_skip(void)
{
    ...
    return tpa_recv(tpa_chan(1), (void **)&w->skip, &w->skip_len, yolo_subgraph_p5_run);
}
...
tpa_op_t yolo_subgraph_p5_run(void)
{
    ...
    YV5N_REQUIRE_MSG_STOP("P5:TOP\n", w->top_len == TOP_H * TOP_W * TOP_C);
    YV5N_REQUIRE_MSG_STOP("P5:SKIP\n", w->skip_len == SKIP_H * SKIP_W * SKIP_C);

    yolov5n_arena_begin(YV5N_P5_SCRATCH_PEAK_BYTES);
    ...
    cat_buf = yolov5n_arena_alloc(NR_PIX * CAT1_C, 64u);
    patch_scratch = yolov5n_arena_alloc(YV5N_P5_PATCH_BYTES, 64u);
    YV5N_REQUIRE_MSG_STOP("P5:ALLOC\n", cat0_buf && mid_buf && a_buf &&
                          b_buf && cat_buf && patch_scratch);

    enable_tensor_scratchpad();

    concat_input_upsampled(w->top, w->skip, cat0_buf);
    YV5N_TRY_MSG_STOP("P5:L40\n", run_conv1x1_layer(&layer40, cat0_buf, CAT0_C, a_buf, BRANCH_C));
    YV5N_TRY_MSG_STOP("P5:L41\n", run_conv1x1_layer(&layer41, a_buf, BRANCH_C, mid_buf, BRANCH_C));
    YV5N_TRY_MSG_STOP("P5:L42\n", run_conv3x3_layer(&layer42, mid_buf, BRANCH_C, b_buf, BRANCH_C));
    add_residual_block(a_buf, b_buf, BRANCH_C * NR_PIX);
    YV5N_TRY_MSG_STOP("P5:L43\n", run_conv1x1_layer(&layer43, cat0_buf, CAT0_C, a_buf, BRANCH_C));
    concat_branch(b_buf, a_buf, cat_buf);
    YV5N_TRY_MSG_STOP("P5:L44\n", run_conv1x1_layer(&layer44, cat_buf, CAT1_C, out_buf, OUTPUT_C));

    return tpa_send(tpa_chan(2), out_buf, NR_PIX * OUTPUT_C, yolo_subgraph_p5_done);
}
