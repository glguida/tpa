...
#define MSG_VAL     0x8877665544332211ull
...
tpa_op_t msg_cross_rf_tx_start(void);
tpa_op_t msg_cross_rf_tx_done(void);
tpa_op_t msg_cross_rf_rx_start(void);
tpa_op_t msg_cross_rf_rx_done(void);
tpa_op_t msg_cross_rf_chk(void);
...
tpa_op_t msg_cross_rf_tx_start(void)
{
    ...
    if (ch->state != TPA_CH_WAIT_RX)
        return tpa_yield(msg_cross_rf_tx_start);

    return tpa_send(ch, (void *)&tx_buf, sizeof(tx_buf),
                    msg_cross_rf_tx_done);
}
...
tpa_op_t msg_cross_rf_rx_start(void)
{
    ...
    return tpa_recv(ch, (void **)&rx_buf, &rx_len, msg_cross_rf_rx_done);
}
...
tpa_op_t msg_cross_rf_chk(void)
{
    ...
    asm volatile("fence rw, rw");
    return tpa_yield(msg_cross_rf_chk);
}
