...
#define MSG_VAL     0x13579bdf2468ace0ull
...
tpa_op_t msg_same_sf_tx_start(void);
tpa_op_t msg_same_sf_tx_done(void);
tpa_op_t msg_same_sf_rx_start(void);
tpa_op_t msg_same_sf_rx_done(void);
tpa_op_t msg_same_sf_chk(void);
...
tpa_op_t msg_same_sf_rx_start(void)
{
    ...
    if (ch->state != TPA_CH_WAIT_TX)
        return tpa_yield(msg_same_sf_rx_start);

    return tpa_recv(ch, (void **)&rx_buf, &rx_len, msg_same_sf_rx_done);
}
...
tpa_op_t msg_same_sf_chk(void)
{
    ...
    asm volatile("fence rw, rw");
    return tpa_yield(msg_same_sf_chk);
}
