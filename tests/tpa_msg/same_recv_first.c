...
#define MSG_VAL     0x0fedcba987654321ull
...
tpa_op_t msg_same_rf_tx_start(void);
tpa_op_t msg_same_rf_tx_done(void);
tpa_op_t msg_same_rf_rx_start(void);
tpa_op_t msg_same_rf_rx_done(void);
tpa_op_t msg_same_rf_chk(void);
...
tpa_op_t msg_same_rf_tx_start(void)
{
    ...
    if (ch->state != TPA_CH_WAIT_RX)
        return tpa_yield(msg_same_rf_tx_start);
...
tpa_op_t msg_same_rf_rx_start(void)
{
    ...
    return tpa_recv(ch, (void **)&rx_buf, &rx_len, msg_same_rf_rx_done);
}
...
tpa_op_t msg_same_rf_chk(void)
{
    ...
    asm volatile("fence rw, rw");
    return tpa_yield(msg_same_rf_chk);
}
