...
#include "generated/yolov5n_l40_44_case0.h"
...
#define YOLO_C3_40_44_BEGIN       0xe3404400u
#define YOLO_C3_40_44_GOOD_INPUT  0xe3404401u
#define YOLO_C3_40_44_GOOD_OUTPUT 0xe3404402u
#define YOLO_C3_40_44_FAIL        0xe34044eeu

#define INPUT_H    YOLOV5N_L40_44_CASE0_INPUT_H
#define INPUT_W    YOLOV5N_L40_44_CASE0_INPUT_W
#define INPUT_C    YOLOV5N_L40_44_CASE0_INPUT_C
...
#define BRANCH_C   32u
#define CAT_C      64u
#define OUTPUT_C   YOLOV5N_L40_44_CASE0_OUTPUT_C
...
tpa_op_t yolo_c3_l40_44_start(void);
...
static int8_t out_buf[YOLOV5N_L40_44_CASE0_OUTPUT_ELEMS]
...
{
    const int8_t *lut = yolov5n_l40_44_res_lut;
...
static void concat_c_int8_2way(const int8_t *a, const int8_t *b, int8_t *dst)
{
...
tpa_op_t yolo_c3_l40_44_start(void)
{
    uint64_t got_in_hash = hash_buf(yolov5n_l40_44_case0_in,
                                    YOLOV5N_L40_44_CASE0_INPUT_ELEMS);
    uint64_t got_hash;
    ...
    mark(YOLO_C3_40_44_BEGIN);

    if (INPUT_C != 128u || BRANCH_C != 32u || CAT_C != 64u || OUTPUT_C != 64u ||
        YOLOV5N_L40_44_CASE0_OUTPUT_ELEMS != NR_PIX * OUTPUT_C) {
        mark(YOLO_C3_40_44_FAIL);
        mark(0x10u);
        TEST_FAIL;
        return tpa_stop();
    }

    if (got_in_hash != YOLOV5N_L40_44_CASE0_INPUT_HASH) {
        mark(YOLO_C3_40_44_FAIL);
        mark(0x11u);
...

    {
        ...
        if (run_conv1x1_layer(&layer40, yolov5n_l40_44_case0_in, INPUT_C,
                              a0_buf, BRANCH_C)) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x20u);
            TEST_FAIL;
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_A0_HASH) {
            diag_puts("a0_hash=");
            diag_putu64(got_hash);
            diag_putc('\n');
            mark(YOLO_C3_40_44_FAIL);
            mark(0x21u);
            TEST_FAIL;
    ...
    {
        uint64_t beg = emu_cycle();
        if (run_conv1x1_layer(&layer41, a0_buf, BRANCH_C, mid_buf, BRANCH_C)) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x22u);
            TEST_FAIL;
            return tpa_stop();
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_MID_HASH) {
            diag_puts("mid_hash=");
            diag_putu64(got_hash);
            diag_putc('\n');
    ...
    {
        uint64_t beg = emu_cycle();
        if (run_conv3x3_layer(&layer42, mid_buf, BRANCH_C, a_buf, BRANCH_C)) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x24u);
            TEST_FAIL;
            return tpa_stop();
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_BODY_HASH) {
            diag_line("C3_40_44_CV40", cv40_cycles);
            diag_line("C3_40_44_CV41", cv41_cycles);
            diag_line("C3_40_44_CV42", cv42_cycles);
            diag_line("C3_40_44_A0CHK", a0chk_cycles);
            diag_line("C3_40_44_MIDCHK", midchk_cycles);
            diag_line("C3_40_44_BODYCHK", bodychk_cycles);
            diag_puts("body_hash=");
            diag_putu64(got_hash);
            diag_putc('\n');
            mark(YOLO_C3_40_44_FAIL);
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_A_HASH) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x26u);
            TEST_FAIL;
            return tpa_stop();
    ...
    {
        uint64_t beg = emu_cycle();
        if (run_conv1x1_layer(&layer43, yolov5n_l40_44_case0_in, INPUT_C,
                              b_buf, BRANCH_C)) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x27u);
            TEST_FAIL;
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_B_HASH) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x28u);
            TEST_FAIL;
            return tpa_stop();
    ...
    {
        uint64_t beg = emu_cycle();
        concat_c_int8_2way(a_buf, b_buf, cat_buf);
    ...
    }
    {
        ...
        if (got_hash != YOLOV5N_L40_44_CASE0_CAT_HASH) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x29u);
            TEST_FAIL;
            return tpa_stop();
    ...
    {
        uint64_t beg = emu_cycle();
        if (run_conv1x1_layer(&layer44, cat_buf, CAT_C, out_buf, OUTPUT_C)) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x2au);
            TEST_FAIL;
            return tpa_stop();
    ...
    {
        uint64_t beg = emu_cycle();
        got_hash = hash_buf(out_buf, YOLOV5N_L40_44_CASE0_OUTPUT_ELEMS);
        if (got_hash != YOLOV5N_L40_44_CASE0_OUTPUT_HASH) {
            mark(YOLO_C3_40_44_FAIL);
            mark(0x2bu);
            mark((uint32_t)got_hash);
            mark((uint32_t)(got_hash >> 32));
            TEST_FAIL;
            return tpa_stop();
        }

        for (size_t i = 0; i < YOLOV5N_L40_44_CASE0_OUTPUT_ELEMS; ++i) {
            int8_t outv = out_buf[i];
            int8_t expv = yolov5n_l40_44_case0_out[i];
    ...
    total_end = emu_cycle();
    diag_line("C3_40_44_TOTAL", total_end - total_beg);
    diag_line("C3_40_44_CV40", cv40_cycles);
    diag_line("C3_40_44_A0CHK", a0chk_cycles);
    diag_line("C3_40_44_CV41", cv41_cycles);
    diag_line("C3_40_44_MIDCHK", midchk_cycles);
    diag_line("C3_40_44_CV42", cv42_cycles);
    diag_line("C3_40_44_BODYCHK", bodychk_cycles);
    diag_line("C3_40_44_ADD", add_cycles);
    diag_line("C3_40_44_ACHK", achk_cycles);
    diag_line("C3_40_44_CV43", cv43_cycles);
    diag_line("C3_40_44_BCHK", bchk_cycles);
    diag_line("C3_40_44_CAT", cat_cycles);
    diag_line("C3_40_44_CATCHK", catchk_cycles);
    diag_line("C3_40_44_CV44", cv44_cycles);
    diag_line("C3_40_44_CHECK", check_cycles);
    diag_line("C3_40_44_TENSOR",
              cv40_cycles + cv41_cycles + cv42_cycles + cv43_cycles + cv44_cycles);
    diag_line("C3_40_44_WORK",
              cv40_cycles + cv41_cycles + cv42_cycles + add_cycles +
              cv43_cycles + cat_cycles + cv44_cycles);

    mark(YOLO_C3_40_44_GOOD_OUTPUT);
    TEST_PASS;
    return tpa_stop();
}
