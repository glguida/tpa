...
#define YOLO_SUBGRAPH_P4_BEGIN       0xe3a44000u
#define YOLO_SUBGRAPH_P4_GOOD_INPUT  0xe3a44001u
#define YOLO_SUBGRAPH_P4_GOOD_OUTPUT 0xe3a44002u
#define YOLO_SUBGRAPH_P4_FAIL        0xe3a440eeu

#define TOP_H      YOLOV5N_P4_P5_P6_CASE0_P5_IN_H
#define TOP_W      YOLOV5N_P4_P5_P6_CASE0_P5_IN_W
#define TOP_C      YOLOV5N_P4_P5_P6_CASE0_P5_IN_C
#define SKIP_H     YOLOV5N_P4_P5_P6_CASE0_P4_SKIP_H
#define SKIP_W     YOLOV5N_P4_P5_P6_CASE0_P4_SKIP_W
#define SKIP_C     YOLOV5N_P4_P5_P6_CASE0_P4_SKIP_C
...
typedef struct {
    ...
    const int8_t *skip;
    uint32_t skip_len;
} yolo_subgraph_p4_ws_t;

tpa_op_t yolo_subgraph_p4_start(void);
tpa_op_t yolo_subgraph_p4_recv_skip(void);
tpa_op_t yolo_subgraph_p4_run(void);
tpa_op_t yolo_subgraph_p4_done(void);
...
static int8_t out_buf[YOLOV5N_P4_P5_P6_CASE0_MID_P4_ELEMS]
    __attribute__((aligned(64)));
/*
 * Keep large 3x3 staging scratch off the hart stack. In the integrated
 * subgraph the neighboring H0 stacks are only 0x800 bytes apart, so a local
 * patch block here corrupts adjacent runtime state while P4 runs.
 */
static int8_t patch_scratch[PIX_BLOCK * 576u] __attribute__((aligned(64)));
...
{
    const int8_t *lut = yolov5n_p4_p5_p6_p4_res_lut;
...
tpa_op_t yolo_subgraph_p4_start(void)
{
    yolo_subgraph_p4_ws_t *w = tpa_ws();
    struct tpa_chan *ch = tpa_chan(0);

    diag_puts("P4_START\n");
    mark(YOLO_SUBGRAPH_P4_BEGIN);

    if (!w || !ch) {
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }

    return tpa_recv(ch, (void **)&w->top, &w->top_len,
                    yolo_subgraph_p4_recv_skip);
}
...
tpa_op_t yolo_subgraph_p4_recv_skip(void)
{
    ...
    return tpa_recv(ch, (void **)&w->skip, &w->skip_len, yolo_subgraph_p4_run);
}
...
tpa_op_t yolo_subgraph_p4_run(void)
{
    ...
    if (!w || !out ||
        w->top_len != sizeof(yolov5n_p4_p5_p6_case0_p5_in) ||
        w->skip_len != sizeof(yolov5n_p4_p5_p6_case0_p4_skip)) {
        diag_puts("FAIL_P4_LEN\n");
    ...
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P5_IN_HASH) {
        fail_hash("p4_top", got_hash, YOLOV5N_P4_P5_P6_CASE0_P5_IN_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        mark(0x10u);
        TEST_FAIL;
        return tpa_stop();
    ...
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_SKIP_HASH) {
        fail_hash("p4_skip", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_SKIP_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        mark(0x11u);
        TEST_FAIL;
        return tpa_stop();
    }
    mark(YOLO_SUBGRAPH_P4_GOOD_INPUT);

    enable_tensor_scratchpad();

    concat_input_upsampled(w->top, w->skip, cat0_buf);
    got_hash = hash_buf(cat0_buf, sizeof(cat0_buf));
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_CAT0_HASH) {
        fail_hash("p4_cat0", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_CAT0_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }

    if (run_conv1x1_layer(&layer34, cat0_buf, CAT0_C, a_buf, BRANCH_C) ||
        run_conv1x1_layer(&layer35, a_buf, BRANCH_C, mid_buf, BRANCH_C) ||
        run_conv3x3_layer(&layer36, mid_buf, BRANCH_C, b_buf, BRANCH_C)) {
        diag_puts("FAIL_P4_CONV\n");
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    ...
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_A0_HASH) {
        fail_hash("p4_a0", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_A0_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    got_hash = hash_buf(mid_buf, sizeof(mid_buf));
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_MID_HASH) {
        fail_hash("p4_mid", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_MID_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    got_hash = hash_buf(b_buf, BRANCH_C * NR_PIX);
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_BODY_HASH) {
        fail_hash("p4_body", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_BODY_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    ...
    got_hash = hash_buf(b_buf, BRANCH_C * NR_PIX);
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_A_HASH) {
        fail_hash("p4_a", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_A_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }

    if (run_conv1x1_layer(&layer37, cat0_buf, CAT0_C, a_buf, BRANCH_C)) {
        diag_puts("FAIL_P4_L37\n");
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    got_hash = hash_buf(a_buf, BRANCH_C * NR_PIX);
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_B_HASH) {
        fail_hash("p4_b", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_B_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    ...
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_CAT1_HASH) {
        fail_hash("p4_cat1", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_CAT1_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    if (run_conv1x1_layer(&layer38, cat1_buf, CAT1_C, cat0_buf, PRE_C)) {
        diag_puts("FAIL_P4_L38\n");
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    got_hash = hash_buf(cat0_buf, NR_PIX * PRE_C);
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_PRE_HASH) {
        fail_hash("p4_pre", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_PRE_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    if (run_conv1x1_layer(&layer39, cat0_buf, PRE_C, out_buf, OUTPUT_C)) {
        diag_puts("FAIL_P4_L39\n");
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }
    ...
    if (got_hash != YOLOV5N_P4_P5_P6_CASE0_P4_OUT_HASH) {
        fail_hash("p4_out", got_hash, YOLOV5N_P4_P5_P6_CASE0_P4_OUT_HASH);
        mark(YOLO_SUBGRAPH_P4_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }

    mark(YOLO_SUBGRAPH_P4_GOOD_OUTPUT);
    return tpa_send(out, out_buf, sizeof(out_buf), yolo_subgraph_p4_done);
}
