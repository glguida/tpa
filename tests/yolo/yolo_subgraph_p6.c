...
#define YOLO_SUBGRAPH_P6_BEGIN       0xe3a46000u
#define YOLO_SUBGRAPH_P6_GOOD_INPUT  0xe3a46001u
#define YOLO_SUBGRAPH_P6_GOOD_OUTPUT 0xe3a46002u
#define YOLO_SUBGRAPH_P6_FAIL        0xe3a460eeu
...
tpa_op_t yolo_subgraph_p6_start(void);
tpa_op_t yolo_subgraph_p6_run(void);
tpa_op_t yolo_subgraph_p6_done(void);

static int8_t det_stage0_buf[NR_PIX * CLS_C] __attribute__((aligned(64)));
static int8_t det_stage1_buf[NR_PIX * CLS_C] __attribute__((aligned(64)));
static yolo_det_hash_msg_t det_msg __attribute__((aligned(64)));
/*
 ...
 */
static int32_t acc_blk_scratch[PIX_BLOCK * OC_BLOCK] __attribute__((aligned(64)));
static int8_t packed_scratch[PIX_BLOCK * 128u] __attribute__((aligned(64)));
static int8_t patch_scratch[PIX_BLOCK * 768u] __attribute__((aligned(64)));
...
tpa_op_t yolo_subgraph_p6_start(void)
{
    yolo_subgraph_p6_ws_t *w = tpa_ws();
    struct tpa_chan *ch = tpa_chan(0);

    diag_puts("P6_START\n");
    mark(YOLO_SUBGRAPH_P6_BEGIN);

    if (!w || !ch) {
        diag_puts("FAIL_P6_START\n");
        mark(YOLO_SUBGRAPH_P6_FAIL);
        TEST_FAIL;
        return tpa_stop();
    }

    return tpa_recv(ch, (void **)&w->in, &w->in_len, yolo_subgraph_p6_run);
}
...
tpa_op_t yolo_subgraph_p6_run(void)
{
    ...
    if (!w || w->in_len != YOLOV5N_P4_P5_P6_CASE0_P3_ELEMS) {
        diag_puts("FAIL_P6_IN\n");
        mark(YOLO_SUBGRAPH_P6_FAIL);
        mark(0x10u);
        TEST_FAIL;
        return tpa_stop();
...
#ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    if (hash_buf(w->in, w->in_len) != YOLOV5N_P4_P5_P6_CASE0_P5_OUT_HASH) {
        fail_hash("input", hash_buf(w->in, w->in_len),
                  YOLOV5N_P4_P5_P6_CASE0_P5_OUT_HASH);
        mark(YOLO_SUBGRAPH_P6_FAIL);
        mark(0x11u);
        TEST_FAIL;
        return tpa_stop();
...
#endif
    mark(YOLO_SUBGRAPH_P6_GOOD_INPUT);
    ...
    b57_beg = emu_cycle();
    diag_puts("P6_57\n");
    if (run_conv3x3_layer_packed(&layer57, w->in, 64u, box1_buf, 64u)) {
        diag_puts("FAIL_P6_57\n");
        mark(YOLO_SUBGRAPH_P6_FAIL);
        mark(0x20u);
        TEST_FAIL;
        return tpa_stop();
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        ...
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_BOX1_HASH) {
            fail_hash("box1", h, YOLOV5N_P4_P5_P6_CASE0_DET_BOX1_HASH);
            mark(YOLO_SUBGRAPH_P6_FAIL);
            mark(0x21u);
            TEST_FAIL;
            return tpa_stop();
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        ...
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_BOX2_HASH) {
            fail_hash("box2", h, YOLOV5N_P4_P5_P6_CASE0_DET_BOX2_HASH);
            mark(YOLO_SUBGRAPH_P6_FAIL);
            mark(0x23u);
            TEST_FAIL;
            return tpa_stop();
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        uint64_t h = hash_buf(box_buf, YOLOV5N_P4_P5_P6_CASE0_BOX_ELEMS);
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_BOX_HASH) {
            fail_hash("box", h, YOLOV5N_P4_P5_P6_CASE0_DET_BOX_HASH);
            mark(YOLO_SUBGRAPH_P6_FAIL);
            mark(0x25u);
            TEST_FAIL;
            return tpa_stop();
    ...
    c63_beg = emu_cycle();
    diag_puts("P6_63\n");
    if (run_conv3x3_layer_packed(&layer63, w->in, 64u, cls1_buf, 80u)) {
        diag_puts("FAIL_P6_63\n");
        mark(YOLO_SUBGRAPH_P6_FAIL);
        mark(0x26u);
        TEST_FAIL;
        return tpa_stop();
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        ...
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_CLS1_HASH) {
            fail_hash("cls1", h, YOLOV5N_P4_P5_P6_CASE0_DET_CLS1_HASH);
            mark(YOLO_SUBGRAPH_P6_FAIL);
            mark(0x27u);
            TEST_FAIL;
            return tpa_stop();
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        ...
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_CLS2_HASH) {
            fail_hash("cls2", h, YOLOV5N_P4_P5_P6_CASE0_DET_CLS2_HASH);
        }
    }
    #endif
    ...
    #ifndef YOLO_INTEGRATED_NO_INTERNAL_CHECKS
    {
        uint64_t h = hash_buf(cls_buf, YOLOV5N_P4_P5_P6_CASE0_CLS_ELEMS);
        if (h != YOLOV5N_P4_P5_P6_CASE0_DET_CLS_HASH) {
            fail_hash("cls", h, YOLOV5N_P4_P5_P6_CASE0_DET_CLS_HASH);
            mark(YOLO_SUBGRAPH_P6_FAIL);
            mark(0x2bu);
            TEST_FAIL;
            return tpa_stop();
    ...
    diag_line("P6_TOTAL", total_end - total_beg);
    ...
    mark(YOLO_SUBGRAPH_P6_GOOD_OUTPUT);
    if (tpa_chan(1)) {
        diag_puts("P6_SEND\n");
        det_msg.box_hash = hash_buf(box_buf, YOLOV5N_P4_P5_P6_CASE0_BOX_ELEMS);
        det_msg.cls_hash = hash_buf(cls_buf, YOLOV5N_P4_P5_P6_CASE0_CLS_ELEMS);
        return tpa_send(tpa_chan(1), &det_msg, sizeof(det_msg), yolo_subgraph_p6_done);
    }
    TEST_PASS;
    return tpa_stop();
}
