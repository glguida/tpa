...
- `docs/limitations.md`
  - Centralize missing/partial status for YOLO block tests, archived DNN/LTFarm
    status, tools, models, full runtime scheduler, and broader metadata
    coverage. Message/queue/negative tests are ported as build targets but need
    full scheduler behavioral validation.
