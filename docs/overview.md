...
- `yolov8n/` — opt-in external-header YOLOv8n downstream milestones for P5-only
  ...
  validation feeding sampled Detect/DFL, with mapper-generated placement/edge
  config.
...
- YOLOv8n external-header mapper/device targets build when explicitly
  ...
  sampled combined P3/P4/P5 C2f+Detect downstream graph, and dense P3/P4/P5 C2f
  feature-map hashes feeding sampled per-scale Detect;
...
C2f+Detect downstream graph, and dense P3/P4/P5 C2f feature-map validation
feeding sampled per-scale Detect; dense combined/full-model validation and the
